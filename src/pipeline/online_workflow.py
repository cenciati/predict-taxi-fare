...
class OnlineInferencePipeline:
    ...
    def execute(self) -> Dict[str, float]:
        ...
        try:
            ...
            result_data: pd.DataFrame = self.__make_prediction(
                preprocessed_data
            )
            ...
            logger.critical(
                '`OnlineInferencePipeline` workflow could not be completed. %s',
                exc,
            )
    ...
    def __prepare_data(self, raw_data: pd.DataFrame) -> pd.DataFrame:
        with disable_run_logger():
            ...
            cleaned_data_with_features: pd.DataFrame = create_features.fn(
                cleaned_data
            )
